...
def upload_to_comfy(path: Path):
    with path.open("rb") as f:
        r = requests.post(f"{COMFYUI_URL}/upload/image", files={"image": (path.name, f, "image/png")}, data={"overwrite": "true"}, timeout=60)
    r.raise_for_status()
    return r.json()["name"]

def queue_prompt(workflow):
    r = requests.post(f"{COMFYUI_URL}/prompt", json={"prompt": workflow, "client_id": str(uuid.uuid4())}, timeout=30)
    r.raise_for_status()
...
def clean_alpha(raw: bytes) -> bytes:
    # rembg preserves the subject while creating a real alpha channel.
    out = remove(raw)
...
def build_prompt(character_name, description, build, bust, waist, hips, legs, pose, clothing, edit):
    ...
    return (
        f"photorealistic adult woman, full body, head to toe, centered, {BODY_WORDS.get(build, BODY_WORDS['average'])}, "
        f"{sliders}, {POSE_WORDS.get(pose, POSE_WORDS['front'])}, "
        f"{clothing.strip() or 'simple neutral fitted clothing'}, "
        f"character identity: {character_name.strip() or 'unnamed character'}, "
        f"{description.strip()} "
        f"BODY AND APPEARANCE EDIT HAS PRIORITY: {edit_text if edit_text else 'preserve the requested body proportions'}; "
        "preserve face, hair, age, skin tone and identity from the reference; do not change identity."
    )
...
async def generate(
    ...
    workflow["2"]["inputs"]["text"] = build_prompt(character_name, description, build, bust, waist, hips, legs, pose, clothing, edit)
